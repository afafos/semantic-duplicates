...
Статистика по собранным датасетам в файлах `view_2_sources` (без Лейпцига) и `view_3_sources` (с Лейпцигом). Вынес отдельно, так как будет большое смещение выборки для положительных примеров именно дублей.
