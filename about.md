...
3. `load_dataset.py` - сборка датасета. Можно добавить флаги, чтобы без какого-то датасета собрать.
3. `train_duplicate_model.py`
Что делает:
Загружает собранный датасет unified_news_pairs.csv (300k+ пар)
...
6. `news_similarity_checker.py` - исходный скрипт от заказчика.

7. `convert_json_to_csv.py` - скрипт для перевода согласованной разметки osokina и popov в csv формат.

8. `view_dataset_stats.py` - Скрипт для просмотра статистики по каждому исходному датасету.
